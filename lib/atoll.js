...
var atoll = function(){
  ...
  // added na_rm as it exist in R
  // If na_rm is FALSE an undefined or NaN value in array will cause a value of undefined or NaN to be returned, otherwise undefined and NaN values are ignored.


  var atoll = function(data, na_rm){ return new coverup(data, na_rm); };
  
  // the coverup is the oop instance of atoll
  var coverup = function(data, na_rm) { 
    var na = na_rm || false;
    if (!na) {
      this._data = data; 
    } else {
